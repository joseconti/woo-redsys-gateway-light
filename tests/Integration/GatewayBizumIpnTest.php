...
class GatewayBizumIpnTest extends WP_UnitTestCase {
	...
	public function test_does_not_crash_on_a_notification_referencing_a_nonexistent_order() {
		...
		// Whether it accepts or rejects isn't the point (the signature can
		// still validate against the gateway's own default secret even for
		// an order that doesn't exist) — the point is that resolving the
		// secret for a nonexistent order must not throw.
