...
| Fail closed when no SHA-256 secret is configured (reject rather than trust an unauthenticated notification) | `IN PLACE` for `class-wc-gateway-redsys.php` and `class-wc-gateway-bizum-redsys.php` only | `classes/class-wc-gateway-redsys.php` ~lines 868–943, **automated-test-verified 2026-08-01** (`tests/Integration/GatewayRedsysIpnTest.php`); `classes/class-wc-gateway-bizum-redsys.php`, same shape, **automated-test-verified 2026-08-01** (`tests/Integration/GatewayBizumIpnTest.php`); confirmed by the 7.0.1/7.0.2 `readme.txt` changelog entries describing this exact hardening; also **driven-verified 2026-08-01** in the wp-env playground for the `redsys` gateway — two fabricated POSTs to `?wc-api=WC_Gateway_redsys` with no configured secret were rejected and the target order stayed `wc-pending`, see `docs/playground.md`. **`class-wc-gateway-googlepay-redirection-redsys.php` does NOT have this control — see "Known vulnerabilities" below.** |
...
## Known vulnerabilities (found 2026-08-01, escalated — NOT fixed, pending the user's decision)

| Vulnerability | Where | Impact | Status |
|---|---|---|---|
| **Signature verification bypass via the public merchant code, when no SHA-256 secret is configured** | `classes/class-wc-gateway-googlepay-redirection-redsys.php`, `check_ipn_request_is_valid()` lines ~882–910 (the `else` branch that runs when `$usesecretsha256` is empty) | Unlike `class-wc-gateway-redsys.php` and `class-wc-gateway-bizum-redsys.php` (which explicitly fail closed in this case — see the code comment "No SHA256 secret configured: fail closed... never the non-secret Ds_MerchantCode (FUC)" in both), the Google Pay gateway instead accepts the notification if the decoded `Ds_MerchantCode` in the (attacker-supplied) `Ds_MerchantParameters` equals `$this->customer` — the merchant's FUC/`DS_MERCHANT_MERCHANTCODE`. That value is **not a secret**: it is sent in plaintext in every outgoing payment form this plugin generates (`set_parameter( 'DS_MERCHANT_MERCHANTCODE', ... )`), so it is trivially readable by anyone who starts a checkout. **If a site leaves the SHA-256 secret field empty for this gateway, any visitor can forge a fake "payment successful" notification for any order** by POSTing a `Ds_MerchantParameters` they built themselves with the known merchant code — no signature required. Found while reading the full method to write its integration tests (the same pattern used for Redsys/Bizum); tests were NOT written for this class's fail-closed path because doing so would either assert vulnerable behavior as correct or require deciding the fix first. | **Escalated to the user 2026-08-01, not yet fixed.** Not silently patched, per Keel protocol ("something looks like a security problem → escalate, never patch quietly"). |
