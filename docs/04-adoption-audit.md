...
> **Update, 2026-08-01 (Phase 5 sprint, post-adoption):** the wp-env playground and PHPUnit coverage recommended above are now built — `RedsysLiteAPI` unit tests (D-016/D-017) and `WC_Gateway_redsys`'s IPN validation integration tests (D-018), see `docs/playground.md` and `docs/03-technical-plan.md` §Testing. This snapshot is left unedited above as the as-adopted record; still open: a Playwright smoke test for the checkout flow, and the same integration-test pattern for the other three gateway classes.
