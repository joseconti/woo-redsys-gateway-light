...
> **Update, 2026-08-01 (Phase 5 sprint, post-adoption):** the wp-env playground and PHPUnit coverage recommended above are now built — `RedsysLiteAPI` unit tests (D-016/D-017) and IPN validation integration tests for `WC_Gateway_redsys` (D-018), `WC_Gateway_Bizum_Redsys` (D-019), and `WC_Gateway_GooglePay_Redirection_Redsys` (D-020 — this last one also uncovered and fixed a real signature-bypass vulnerability, see `docs/threat-model.md` "Known vulnerabilities"). See `docs/playground.md` and `docs/03-technical-plan.md` §Testing. This snapshot is left unedited above as the as-adopted record; still open: a Playwright smoke test for the checkout flow, and a test suite for `class-wc-gateway-inespay-redsys.php` (different signature algorithm, needs its own fixture).
