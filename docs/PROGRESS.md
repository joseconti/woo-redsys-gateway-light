...
- Next action: this session found and fixed one real security/correctness bug beyond the earlier GooglePay fix (D-023) that is unreleased — a version bump + Phase 7 release should now be a priority so both reach production sites, rather than sitting fixed-but-unshipped on `develop`. 8 further findings from the full-plugin review (D-024's 7 lower-severity items, plus D-025's plaintext-secret-in-order-meta item) are documented and deliberately deferred, not fixed this session — user scoped this session's fixes to D-020 and D-023 only. All 8 open GitHub issues have now been triaged for real (see `docs/issues.md`): 7 turned out to be stale/already-resolved (commented on GitHub, left open for the reporter/maintainer to close), 1 (#93) got a real fix this session. GitHub also reported 55 Dependabot alerts on the default branch after this session's push — not investigated yet, unrelated to any dependency this session touched.

## Open items
- Unresolved user questions: none — the plaintext-secret-in-order-meta finding was resolved as "document, defer" (D-025).
...
- **The plaintext-secret-in-order-meta finding (D-025)** — Bizum and GooglePay persist the real Redsys signing secret, unencrypted and permanently, to order postmeta. Documented in `docs/threat-model.md`; user was offered three fix approaches and chose to leave it documented and decide later rather than guess at a fix that could break the per-user test-mode feature it's load-bearing for. Review trigger: whenever the user is ready to decide.
- **7 further findings from the full-plugin review (D-024), documented, not fixed this session** — user scoped the session to the 2 "Alto" findings only:
