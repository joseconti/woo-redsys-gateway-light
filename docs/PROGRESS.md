...
| 5 Development | scaffold complete (D-014), playground verified for real; slices done: #93 fix, RedsysLiteAPI unit tests (D-016), IPN/callback integration tests for all four gateways — redsys (D-018), Bizum (D-019), GooglePay incl. security fix (D-020), Inespay (D-021) | docs/issues.md, docs/playground.md, scripts/keel-doctor, scripts/keel-verify, scripts/keel-handoff-verify, docs/sprints/, docs/05-test-points.md, docs/threat-model.md, composer.json, phpunit.xml.dist, phpunit-integration.xml.dist, tests/, classes/class-wc-gateway-googlepay-redirection-redsys.php |
...
## Current position
- Phase: Adoption complete, Phase 5 scaffold built and playground verified for real (D-014); slices done: #93 fix, `RedsysLiteAPI` unit tests (D-016/D-017), and IPN/callback integration tests for all four gateway classes — `WC_Gateway_redsys` (D-018), `WC_Gateway_Bizum_Redsys` (D-019), `WC_Gateway_GooglePay_Redirection_Redsys` including a real security fix (D-020), `WC_Gateway_Inespay_Redsys` (D-021). 28 tests total (8 unit + 20 integration), every one mutation- or regression-verified.
- Next action: this session found and fixed a real security bug (D-020) that is unreleased — a version bump + Phase 7 release should now be a priority so it reaches production sites, rather than sitting fixed-but-unshipped on `develop`. Confirm with the user whether to cut that release now, or continue further work first (a Playwright smoke test for the checkout flow is the last open testing gap). All 8 open GitHub issues have now been triaged for real (see `docs/issues.md`): 7 turned out to be stale/already-resolved (commented on GitHub, left open for the reporter/maintainer to close), 1 (#93) got a real fix this session. GitHub also reported 55 Dependabot alerts on the default branch after this session's push — not investigated yet, unrelated to any dependency this session touched.
...
- Automated test coverage: `RedsysLiteAPI` (D-016) and all four gateway classes' IPN/callback validation are covered — `WC_Gateway_redsys` (D-018), `WC_Gateway_Bizum_Redsys` (D-019), `WC_Gateway_GooglePay_Redirection_Redsys` (D-020, including the security-fix regression test), `WC_Gateway_Inespay_Redsys` (D-021). Only a checkout-flow (Playwright) smoke test remains open — no JS test suite exists at all. Flagged in `docs/04-adoption-audit.md` (Testability), severity low (was high — every notification/callback signature-verification path is now covered), review trigger "before the next release, or when the checkout flow's JS is next touched"
...
Last updated: 2026-08-01 — Phase 5 sprint (test coverage for RedsysLiteAPI and all four gateway classes; GooglePay signature-bypass vulnerability found, escalated, and fixed — 28 tests total)
