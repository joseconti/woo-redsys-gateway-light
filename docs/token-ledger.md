...
| 2026-08-01 | Phase 5 sprint: PHPUnit test suites (all 4 gateways) + GooglePay security fix | Claude Sonnet 5 | Unit tests for `RedsysLiteAPI` (D-016/D-017); integration tests for `WC_Gateway_redsys` (D-018), `WC_Gateway_Bizum_Redsys` (D-019, real WC_Order fixture), `WC_Gateway_GooglePay_Redirection_Redsys` (D-020), `WC_Gateway_Inespay_Redsys` (D-021, own signature algorithm + WPDieException); found + escalated + fixed a real signature-bypass vulnerability in the GooglePay gateway (regression-tested against the pre-fix code); 28 tests total, all mutation/regression-verified. L-003 and L-004 recorded. 7 commits pushed to `develop`. Security fix unreleased — flagged as a release priority. |
