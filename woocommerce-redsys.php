...
?>
			<h3><?php _e( 'Servired/RedSys Spain', 'woo-redsys-gateway-light' ); ?></h3>
			...
			<p><?php _e( 'Servired/RedSys works by sending the user to your bank TPV to enter their payment information.', 'woo-redsys-gateway-light' ); ?></p>
...
?>
		...
		function receipt_page( $order ) {
			echo '<p>'.__( 'Thank you for your order, please click the button below to pay with Credit Card via Servired/RedSys.', 'woo-redsys-gateway-light' ).'</p>';
		...
		function check_ipn_response() {
			...
			if ( $this->check_ipn_request_is_valid() ) {
				...
				do_action( "valid-redsys-standard-ipn-request", $_POST );
			} else {
	...
	function add_redsys_meta_box(){
		...
		echo '<p><strong>' . __( 'Redsys Date', 'woo-redsys-gateway-light' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_payment_date_redsys', true) . '</p>';
		echo '<p><strong>' . __( 'Redsys Hour', 'woo-redsys-gateway-light' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_payment_hour_redsys', true) . '</p>';
		echo '<p><strong>' . __( 'Redsys Authorisation Code', 'woo-redsys-gateway-light' ) . ': </strong><br />' . get_post_meta( get_the_ID(), '_authorisation_code_redsys', true) . '</p>';
	}
	add_action( 'woocommerce_admin_order_data_after_billing_address', 'add_redsys_meta_box' );

	// Adding Iupay
	require_once plugin_dir_path( __FILE__ ) . 'woocommerce-redsys-iupay.php';
}
?>
