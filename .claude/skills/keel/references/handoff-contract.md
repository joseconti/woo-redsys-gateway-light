...
10. **The handoff directory is Design's delivery and NOTHING else — it must stay wholesale-replaceable.** `docs/design/design-handoff/` contains exclusively the bytes Design delivered (or the recorded no-Design branch produced in its place). Nothing else is EVER written into it — no build output, no notes or audit evidence, no user-acquired fonts, no generated assets, no scripts, no editor droppings. The reason is operational and field-tested: installing a delivery or a re-delivery means removing the whole directory and placing the new one — a single foreign file makes that swap unsafe and blocks it. Everything Keel-side already has its home elsewhere: the brief at `docs/design/DESIGN-BRIEF.md`, Design Requests at `docs/design/design-requests/`, the consolidated contract and audit evidence in `docs/BUILD-SPEC.md`, and user-generated assets or acquired fonts in the PROJECT's own tree (Phase 4 Step 6). The Phase 4 audit flags any file the delivery's own SPEC does not account for as foreign.
