...
class WC_Gateway_Iupay extends WC_Payment_Gateway {
	...
	*/
	public function __construct() {
		...
		// Actions.
		add_action( 'valid_iupay_standard_ipn_request', array( $this, 'successful_request' ) );
	...
	function checkfor254testiupay() {
		...
		if ( ! $usesecretsha256 && 'yes' === $iupayactive ) {
			$checkfor254 = true;
		} else {
			$checkfor254 = false;
		}
	...
	*/
	public function admin_options() {
		?>
		<h3><?php _esc_html_e( 'Iupay', 'woo-redsys-gateway-light' ); ?></h3>
		<p><?php _esc_html_e( 'Iupay works by sending the user to your bank TPV to enter their payment information.', 'woo-redsys-gateway-light' ); ?></p>
	...
	function get_iupay_args( $order ) {
		...
		if ( class_exists( 'SitePress' ) ) {
			if ( 'es' === ICL_LANGUAGE_CODE ) {
				...
				$gatewaylanguage = '002';
			} elseif ( 'ca' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '003';
			} elseif ( 'fr' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '004';
			} elseif ( 'ge' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '005';
			} elseif ( 'nl' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '006';
			} elseif ( 'it' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '007';
			} elseif ( 'sv' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '008';
			} elseif ( 'pt' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '009';
			} elseif ( 'pl' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '011';
			} elseif ( 'gl' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '012';
			} elseif ( 'eu' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '013';
			} elseif ( 'da' === ICL_LANGUAGE_CODE ) {
				$gatewaylanguage = '108';
			} else {
				$gatewaylanguage = '002';
			}
	...
	*/
	function receipt_page( $order ) {
		echo '<p>' . esc_html__( 'Thank you for your order, please click the button below to pay with Iupay.', 'woo-redsys-gateway-light' ) . '</p>';
		echo esc_attr( $this->generate_iupay_form( $order ) );
	}
	/**
	...
	*/
	function check_ipn_response() {
		...
		if ( $this->check_ipn_request_is_valid() ) {
			...
			do_action( 'valid_iupay_standard_ipn_request', $_POST );
		} else {
	...
	function successful_request( $posted ) {
		...
		if ( $response <= 99 ) {
			...
			if ( $order_total_compare !== $total ) {
				...
				/* translators: order and received amound */
			...
			if ( ! empty( $dscargtype ) ) {
				update_post_meta( $order->get_id(), '_card_type_iupay', 'C' === $dscargtype ? 'Credit' : 'Debit' );
			}
			// Payment completed.
...
function add_iupay_meta_box() {
	...
	echo '<p><strong>' . esc_html__( 'Iupay Date', 'woo-redsys-gateway-light' ) . ': </strong><br />' . esc_attr( get_post_meta( get_the_ID(), '_payment_date_iupay', true ) ) . '</p>';
	echo '<p><strong>' . esc_html__( 'Iupay Hour', 'woo-redsys-gateway-light' ) . ': </strong><br />' . esc_attr( get_post_meta( get_the_ID(), '_payment_hour_iupay', true ) ) . '</p>';
	echo '<p><strong>' . esc_html__( 'Iupay Authorisation Code', 'woo-redsys-gateway-light' ) . ': </strong><br />' . esc_attr( get_post_meta( get_the_ID(), '_authorisation_code_iupay', true ) ) . '</p>';
}
